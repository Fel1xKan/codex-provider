...
from lib.common.platform import run_editor as default_run_editor
...
def ensure_registry_ready() -> None:
    ...
    if not state.providers:
        raise SwitchError("no providers configured; add one first with `clpx add`")
...
def print_list() -> int:
    ...
    for name in ordered:
        ...
        model_suffix = f" (model={model})" if isinstance(model, str) and model else ""
        print(f"{marker} {name} (auth={auth_status}){model_suffix}")
    return 0
...
def print_status() -> int:
    ...
    print(f"settings file: {state.settings_path}")
...
def auth_target_path(provider: str | None) -> tuple[str | None, Any]:
    if provider is None:
        state = st.ensure_provider_state(read_only=True)
        return state.active_provider or None, None
    provider = validate_provider_name(provider)
...
def show_auth(provider: str | None) -> int:
    ...
    print(f"auth file: {path} (profile '{target}')")
...
def edit_auth(provider: str | None) -> int:
    ...
    if path is None or not path.exists():
        raise SwitchError(
            f"auth profile not found; run `clpx auth edit {target}` "
