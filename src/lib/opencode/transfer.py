...
def export_command(file_path: str | None) -> int:
    ...
    if not file_path or file_path == "-":
        ...
        print(f"exported OpenCode configuration and auth to {path}")
    return 0
...
def import_command(file_path: str | None, dry_run: bool) -> int:
    ...
    if data.get("type") != "opencode-provider":
        t = data.get("type")
        raise SwitchError(
            f"invalid export file type: expected opencode-provider, found {t}"
        )
